...
createdBy.innerHTML = createdBy.innerHTML + " Matias Cox";
...
function buildToDo(todo, index) {
  //todo={description: description, complete: true/false} osea es un objeto con dos propiedades!
  // Tu código acá:
  ...
  toDoText.id = index; //equivalente a toDoText.setAttribute("id,index")

  if (todo.complete)  toDoText.className = "completeText" ;//equivalente a toDoText.classList.add("completeText")
  

  toDoShell.appendChild(toDoText); //en esta linea le estamos asignando a toDoShell un nodo hijo toDoText
...
function buildToDos(toDos) {
  // Tu código acá:
  //toDos es un arreglo de objecto [{descriptiom, comprete}, {description, complete }]

  var arr=toDos.map(function(todo,i) { //esto significa que por cada elemento en el array lo procesara con buildToDo
...
function addToDo() {
  ...
  if (input.value !== "") {
    ...
    input.value = "" //me queda el input vacido "UX"
...
add.addEventListener("click",addToDo); //aca va sin parentesis porque sino se va a ejecutar la funcion
...
function completeToDo(event) {
  ...
  const index = event.target.id; //NO entendi muy bien esta linea
  //event-click
  //event.target - span
  //event.target.id - index buildToDo //index del span
  // Tu código acá:
  toDoItems[index].completeToDo(); //podemos acceder a los elementos a un array con arr.at(indice) //podemos poner (-1) si es es que queremos acceder al ultimo elemento
  displayToDos();
